                  ...
                  <td className="py-2.5 px-2 text-center">
                    {r.paymentFact > 0 ? (
                      <div className="flex items-center justify-center gap-1">
                        {r.paymentDocFileId ? (
                          <button
                            onClick={(e) => { e.stopPropagation(); downloadFile(r.paymentDocFileId, `Платёжка_${r.customerPo}.pdf`); }}
                            className="text-emerald-700 hover:text-emerald-800 font-mono text-xs font-semibold underline-offset-2 hover:underline"
                            title="Скачать платёжку"
                          >
                            ₽{fmt(r.paymentFact)}
                          </button>
                        ) : (
...
const BankImport = ({ balances, setBalances, openPo, setOpenPo, finResults, setFinResults, infraData, setInfraData, debts, setDebts, pushLog }) => {
  ...
  const reset = () => {
    ...
    setAiReasoning(""); setAiConfidence(""); setFinMatches([]); setSelectedFin(null);
  ...
  const handleUploadAndParse = async () => {
    ...
    try {
      const res = await api.parsePayment(file, documentKind);
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3">
        <button
          type="button"
          onClick={() => switchDocumentKind("rf")}
          className={`rounded-xl px-5 py-4 transition-all focus:outline-none border ${
            !isForeign
              ? "bg-[#1E3A5F] text-white shadow-lg border-[#1E3A5F]"
              : "bg-white text-gray-700 border-gray-200 hover:border-[#1E3A5F]/40 hover:shadow-sm"
          }`}
        >
          <span className="flex items-center gap-3 text-sm font-semibold">
            <svg width="24" height="16" viewBox="0 0 24 16" className="rounded-sm flex-shrink-0" aria-hidden="true">
        ...
        <button
          type="button"
          onClick={() => switchDocumentKind("foreign")}
          className={`rounded-xl px-5 py-4 transition-all focus:outline-none border ${
            isForeign
              ? "bg-[#1E3A5F] text-white shadow-lg border-[#1E3A5F]"
              : "bg-white text-gray-700 border-gray-200 hover:border-[#1E3A5F]/40 hover:shadow-sm"
      ...
      {step === 0 && (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
          <h3 className="text-[#1E3A5F] font-semibold text-base mb-4 flex items-center gap-2">
            <span className="w-8 h-8 bg-[#1E3A5F]/12 rounded-lg flex items-center justify-center text-lg">📄</span>
            {isForeign ? "Загрузите иностранный платёжный документ" : "Загрузите платёжный документ РФ"}
          </h3>
          <div className="bg-[#1E3A5F]/06 border border-[#1E3A5F]/20 rounded-lg p-4 mb-5 text-sm text-[#1E3A5F]">
            ...
            <p className="mt-1">
              {isForeign
                ? "AI автоматически распознает данные из документа любого иностранного банка"
      ...
      {step === 1 && editParsed && (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {isForeign ? renderField("Комиссия банка", "fees", "number") : renderField("Номер УПД", "updNumber")}
            {renderField("Дата платежа", "date", "date")}
            {renderField(isForeign ? "Референс / номер ПО" : "Договор / номер заказа", "reference")}
            {renderField("Банк", "bankName")}
            {!isForeign && renderField("Назначение платежа", "purpose")}
          </div>
      ...
      {step === 3 && result && (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
          ...
          <div className="space-y-2 text-sm text-gray-600 mb-6">
            {result.poUpdated && <p>Open PO обновлён — <strong className="text-emerald-600">{isForeign ? "оплата поставщику: Paid" : "оплата клиента: Paid"}</strong></p>}
            {result.finUpdated && <p>Фин. результат обновлён — <strong className="text-emerald-600">₽{fmtRub(parseFloat(editParsed.amount) || 0)}</strong></p>}
            ...
            {result.infraOpCreated && <p>Операция записана в <strong>{selectedAccount}</strong> — новый баланс: <strong className="text-[#1E3A5F]">${fmt(result.newBalance)}</strong></p>}
          </div>
